...
- requires: Equipment 需要 Consumable（设备运行需要特定耗材）
...
3. **必须为每个 Experiment 至少创建一个 uses 或 based_on 关系**
4. 找关系时思考：这个实验用了什么设备？基于什么理论？工具需要什么耗材？
5. confidence 是 0.0 到 1.0 之间的浮点数，表示你对这个关系的确信程度
...
3. **必须为每个 Experiment 类型的实体创建至少 2 个关系（uses 或 based_on）**
4. 对于每个 Equipment 类型的实体，考虑它是否 relates to 某个 Experiment
5. 如果实体列表中有多个 Experiment，考虑它们之间是否有 similar_to 关系
6. 不要输出空数组 []，如果找不到关系请重新仔细阅读实体列表再试"""
...
async def extract_entities_llm(text: str) -> dict:
    ...
    if len(entities) >= 2 and len(relations) == 0:
        ...
        # Build entity listing for the relations-only prompt
        entity_lines = []
        for e in entities:
        ...
        # The relations-only output is an array; parse it
        try:
            ...
            if isinstance(extra_relations, list):
                ...
                logger.info(f"Second pass extracted {len(relations)} relations from dict")
        except json.JSONDecodeError:
            logger.warning(
                "Second pass JSON parse failed. Raw response (first 400 chars): %s",
                relations_response[:400]
            )
            # Try repair
            extra_relations = _parse_json_with_retry(rel_json_str)
            if isinstance(extra_relations, list):
                relations = extra_relations
                logger.info(f"Second pass (repaired) extracted {len(relations)} relations")
            elif isinstance(extra_relations, dict) and "relations" in extra_relations:
                relations = extra_relations["relations"]
                logger.info(f"Second pass (repaired) extracted {len(relations)} relations from dict")
        if len(relations) == 0:
            logger.warning(f"Second pass still returned 0 relations for {len(entities)} entities")
