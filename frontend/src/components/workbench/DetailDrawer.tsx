      ...
      <AnimatePresence>
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {/* Preview button (click to open modal) — enhanced with glow + animation */}
            {card.file_type === "pdf" && (
              <button
                onClick={openPreview}
                disabled={previewLoading}
                className="group relative w-full overflow-hidden rounded-xl border-2 border-[#9A8C73]/30 bg-gradient-to-r from-[#9A8C73]/10 to-[#8C7D70]/5 py-2.5 text-xs font-bold text-[#492D22] shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-[#9A8C73]/50 hover:shadow-md disabled:opacity-50"
                style={{ animation: "preview-pulse 3s ease-in-out infinite" }}
              >
                {/* Sheen effect */}
                <span className="pointer-events-none absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/25 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
                <span className="relative flex items-center justify-center gap-2">
                  <svg className="h-4 w-4 text-[#9A8C73] transition-transform duration-200 group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                  </svg>
                  {previewLoading ? "加载中..." : "📄 在线预览文档"}
                </span>
              </button>
            )}
