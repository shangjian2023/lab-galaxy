...
const STATUS_MAP: Record<string, { label: string; color: string; icon: string }> = {
  uploaded:   { label: "已上传",     color: "bg-gray-100 text-gray-600",   icon: "" },
  parsing:    { label: "AI 解析中",  color: "bg-yellow-100 text-yellow-700", icon: "animate-pulse" },
  extracting: { label: "写入图谱",   color: "bg-blue-100 text-blue-700",   icon: "animate-pulse" },
  completed:  { label: "已完成",     color: "bg-green-100 text-green-700", icon: "" },
  failed:     { label: "处理失败",   color: "bg-red-100 text-red-600",     icon: "" },
};
                    ...
                    >
                      ...
                      <td className="py-2.5 pr-3">
                        <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${s.color}`}>
                          {s.icon && <span className={`h-1.5 w-1.5 rounded-full bg-current ${s.icon}`} />}
                          {s.label}
                        </span>
                      </td>
